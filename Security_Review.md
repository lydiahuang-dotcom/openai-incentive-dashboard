...
**Feature Overview:** The Incentive Dashboard is an internal Google Apps Script web application that provides trainers with a personalized view of their weekly task completions, incentive pay tiers, consistency multipliers, and a ranked leaderboard. Data is sourced from a single Google Sheet (`Gold_Testing`); optional server-side scripts sync Airtable bases (e.g., Hubstaff/production data and Agent Roster) into other sheets. The web app enforces access control by allowing only users whose email appears in the sheet to view the dashboard, and it executes as the accessing user within the Google Workspace domain.

---

**Out of Scope:** This review does not cover: (1) security of the Google Sheets or Google Workspace tenant configuration (sharing, IAM, domain restrictions); (2) security of Airtable bases, views, or API keys outside of how they are used in this project; (3) security of the clasp/local development toolchain; (4) physical or procedural controls for who is added to the roster sheet or who can run sync scripts; (5) third-party browser extensions or client-side modifications after the page is loaded.
...
- **Internal data:** Sheet names, column layout, and all cell data read from `Gold_Testing` (timestamps, emails, status, completion counts); data written to Hubstaff and Roster sheets by sync scripts; Airtable base/table/view identifiers and sync state (e.g., last sync timestamp in a cell).
- **Third-party data:** Records fetched from Airtable via API (Hubstaff sync and Agent Roster pull); content is normazed and written into Google Sheets. Credentials for Airtable (and any Hubstaff) are intended to be stored in PropertiesService (per project context).

**What is the impact if this feature is compromised or misused?**

- **Confidentiality:** Exposure of trainer emails, completion counts, and rankings to unauthorized parties; possible exposure of internal sheet structure or Airtable metadata.
- **Integrity:** Unauthorized changes to sheet data could misstate incentives or leaderboard; sync scripts could overwrite or append incorrect data if credentials or logic are abused.
- **Availability:** Abuse of the web app or sync scripts could lead to quota exhaustion, rate limiting, or temporary unavailability of the dashboard or syncs.
...
- **Network boundary details:** Clients interact with Google’s infrastructure (Apps Script backend, Sheets); the server-side code uses `UrlFetchApp` to call Airtable’s API. There are no custom servers or open listening ports; egress is to Google and Airtable only.
...
| `getDashboardData()` | `google.script.runClient-side JS | None | Single server entry for dashboard data; identity from `Session.getActiveUser().getEmail()`. |
| `syncAirtableIncremental()` | Apps Script (trigger/manual) | Time-driven or user | None | Fetches Airtable; URLs built from constants. |
| `ATRosterPullOverwrite()` | Apps Script (trigger/manual) | Time-driven or user | None | Fetches Airtable; URLs built from constants. |
...
- **SSRF (CWE-918):** Low for the web app—`getDashboardData()` takes no URL or user input. Sync scripts build Airtable URLs from fixed constants; no user-controlled URLs are passed to `UrlFetchApp`. Remaining risk is limited to misconfiguration (e.g., wrong base ID) or future channtroduce user-controlled URLs.
- **Injection (A03, CWE-20/89/78/94):** No SQL or shell; no user input passed to queries. Airtable `filterByFormula` uses a server-controlled ISO timestamp; `encodeURIComponent` is used for table/view/field names. Risk is low.
- **Sensitive data exposure (A02, CWE-200):** Error messages are shown to the client via `withFailureHandler` (e.g., sheet name in “Sheet not found”); exception logging goes to Stackdriver. Credentials must remain only in PropertiesService (see Consolidated Risk List).
...
flowchart LR
  ...
  subgraph Script_Trust
    ...
    SyncHub[Sync Airtable Hubstaff]
    SyncRoster[Sync Roster]
  end
...
**Boundary summary:** The browser is untrusted; the user identity boundary is at Google Auth. Apps Script code and Sheets are inside the Google Workspace boundary. PropertiesService holds secrets; Airtable is an external trust boundary. The dashd does not accept user-supplied parameters; sync scripts are invoked by triggers or editors, not by the web client.
...
| Airtable / Hubstaff API tokens | PropertiesService (intended) | High | Must not be in source; read at runtime only |
| Sheet structure (names, columns) | Code + Sheet | Low–Medium | Error messages may reveal sheet name |
| Sync state (e.g., last sync time) | Sheet cell (e.g., AB1) | Low | Used only for incremental sync |
...
- **Deserialization:** `JSON.parse(response.getContentText())` in sync scripts operates on Airtable API responses only; not user input. Low risk.
...
**Assessment:** No user-controlled input to server logic; injection risk is low. Sync scripts use fixed identifiers and `encodeURIComponent` for dynamic parts of URLs.
...
- **TLS:** All client traffic to Apps Script and Sheets is over HTTPS (Google-managed). `UretchApp` calls to Airtable use `https://api.airtable.com`. No custom TLS configuration in code.
- **Encryption at rest:** Sheets and script project storage are under Google’s control; PropertiesService is Google-managed storage for script properties.
- **Keys / certificates:** No custom key or cert handling; Airtable tokens are the only application secrets (intended to be in PropertiesService).
- **Hardcoded keys:** **Critical:** In the current codebase, `Hubstaff.js` and `Roster.js` contain hardcoded Airtable tokens. Per project context, credentials are to be in PropertiesService; any hardcoded secrets must be removed before production and from history.
...
**Assessment:** TLS and storage are handled by the platform. The main action is to ensure no secrets in source and to use PropertiesService (or equivalent) only.

---

## Secrets Management

- **Approved vault:** Project uses Google Apps Script PropertiesService for Airtable (and Hubstaff) credentials. There is no Doppler or external vault in scope; PropertiesService is the approved store for this app.
- **Secrets in code / env / config / pipelines:** Current repo contains hardcoded `AIRTABLE_TOKEN` and `ROSTER_TOKEN` in `Hubstaff.js` and `Roster.js`. These must be removed; sync functions should call `PropertiesService.getScriptProperties().getProperty('AIRTABLE_TOKEN')` (or similarly named keys) at runtime. No `.env` or config files with secrets should be committed.
- **Rotation:** No aumation in code; rotation is procedural (update property in Apps Script, redeploy if needed). Recommend documenting a rotation procedure.
- **Scoping:** Airtable tokens should have minimal required scopes (e.g., read-only for pull-only use) and be restricted to the specific bases/tables in use.

**Assessment:** Design (PropertiesService) is appropriate. Implementation must remove all hardcoded secrets and use script properties only; add a note in the Consolidated Risk List.
...
- **Debug endpoints:** No debug routes; only `doGet` and `getDashboardData` exposed to the client. Sync functions are not exposed as web endpoints.
- **Default credentials:** None in code; credentials are intended to come from PropertiesService.
...
- **SSRF:** `UrlFetchApp` is used only in sync scripts with URLs built from constants; no user or client input in URLs. Metadata service (e.g., cloud metadata) is not called. SSRF risk is low.
...
- **Secrets injection:** Secrets should be set in Script Properties (UI or clasp) and read at runtime; no containers or VMsork / firewall:** No custom network policies; traffic is to Google and Airtable. Domain restriction limits who can open the web app.
- **Container / image:** N/A; serverless Apps Script runtime.
- **New endpoints:** Only one web-visible server function (`getDashboardData`); sync functions are not HTTP endpoints. Adding new `google.script.run` endpoints would expand attack surface and should be reviewed.
...
|---|-------------------|----------|-------|-----|-------------------------|--------|
| 1 | Hardcoded Airtable (and any Hubstaff) API tokens in `Hubstaff.js` and `Roster.js` | **Critical** | A02 | CWE-798 | Remove all tokens from source; read credentials at runtime from `PropertiService.getScriptProperties()`. Ensure no secrets in git history (consider history rewrite or secret rotation). | Low |
| 2 | User-facing error messages expose internal details (e.g., sheet name “Gold_Testing”) | **Medium** | A04 | CWE-209 | Return generic messages to the client (e.g., “Something went wrong”); log full message/stack in Stackdriver or avoid including sensitive identifiers in thrown messages. | Low |
| 3 | `innerHTML` used for `avgText` with interpolated value; low risk today but fragile | **Low** | A03 | CWE-79 | Set text via `textContent` or ensure `avgNum` is always a number or a fixed safe string; use a `<b>` element for emphasis without raw HTML interpolation. | Low |
| 4 | XFrameOptions set to DEFAULT (allows same-origin framing); possible clickjacking | **Low** | A05 | CWE-1021 | If the  not be embedded, use `HtmlService.XFrameOptionsMode.DENY`. Otherwise document that same-origin framing is acceptable. | Low |
| 5 | No application-level rate limiting on `getDashboardData()` | **Low** | A04 | CWE-770 | Rely on Google quotas or add optional rate limiting (e.g., cache per user with expiration) if abuse is observed. Document reliance on platform quotas. | Medium |
| 6 | Roster (email allow-list) and sheet edit permissions govern access | **Medium** | A01 | CWE-284 | Document who can edit the `Gold_Testing` sheet and roster column; use Sheet permissions and/or audit logs to prevent unauthorized roster changes. | Process |
| 7 | No audit logging of dashboard access or sync runs | **Low** | A09 | CWE-778 | Rely on Apps Script execution logs and Google Admin; or add minimal, non-PII logging (e.g., “dashboard loaded”) if compliance requires it. | Low–Medium |

---

# Reviewer Notes (Security Architect)

- **Scope:** This review is based on the codebase as provided and assumes credentials are or will be stored only in PropertiesService. Any remaining hardcoded tokens must be removed and keys rotated before treating t as production-ready.
- **Architecture:** The design is appropriate for an internal, domain-restricted dashboard: single server entry, no user-supplied parameters, identity from Google Session, and roster-based authorization. The main residual risks are secrets in source, minor information disclosure in errors, and dependency on sheet/roster governance.
- **Next steps:** (1) Remove all secrets from code and use PropertiesService; (2) Sanitize user-facing error messages; (3) Optionally harden `avgText` and XFrameOptions; (4) Document roster and sheet ownership and deployment/rotation procedures.
- **Re-review:** Recommend a lightweight re-check after secrets are removed and if new server-side entry points or user inputs are added.
